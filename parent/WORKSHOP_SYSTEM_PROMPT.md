...
- Workshop adalah **Visual Operating System for Software** â€” sebuah node-based IDE yang mengubah ide menjadi sistem (bukan sekadar produk).
...
- Ide â†’ Sistem (bukan Ide â†’ Produk). User membangun mesin, bukan sekadar landing page.
...
- Bi-directional Sync: Ubah di canvas â†’ kode berubah. Edit kode â†’ canvas update.
...
- Mereka: chat â†’ app (black box, hidden structure).
- Workshop: chat â†’ visual system â†’ code (transparent, editable, ownable).

## 4. ARSITEKTUR TEKNIS YANG HARUS KAMU DESAIN

### 4.1 Monorepo Structure (Tambahkan ke `project-rain-build/`)
packages/workshop/ â”œâ”€â”€ canvas/ # React Flow wrapper + custom nodes â”‚ â”œâ”€â”€ nodes/ # PageNode, ComponentNode, FunctionNode, QueryNode, TableNode â”‚ â”œâ”€â”€ edges/ # Edge types (data flow, trigger, relation) â”‚ â””â”€â”€ hooks/ # useCanvas, useNodeSelection, useLayout â”œâ”€â”€ codegen/ # AST-based transformer â”‚ â”œâ”€â”€ parsers/ # JSON Ground Truth â†’ AST â”‚ â”œâ”€â”€ generators/ # AST â†’ React / Python / SQL â”‚ â””â”€â”€ templates/ # Template engine (Next.js page, API route, Prisma schema) â”œâ”€â”€ sandbox/ # Isolated preview environment â”‚ â”œâ”€â”€ docker/ # Dockerfile untuk sandbox container â”‚ â”œâ”€â”€ preview-server/ # Mini Next.js runner inside container â”‚ â””â”€â”€ websocket/ # Hot reload dari host ke sandbox â”œâ”€â”€ agents/ # Workshop-specific agents â”‚ â”œâ”€â”€ survey/ # Requirement gathering (Sonnet 4.6 level) â”‚ â”œâ”€â”€ drizzle/ # Architecture validation (kamu/konsultan) â”‚ â”œâ”€â”€ storm/ # Code generation (DeepSeek V4 Pro untuk boilerplate, Sonnet untuk complex) â”‚ â””â”€â”€ review/ # Quality check & conflict resolution â””â”€â”€ shared/ # Types, constants, Ground Truth schema â”œâ”€â”€ types/ â””â”€â”€ schema/
...
Canvas â†’ Code: On node add/update/delete â†’ regenerate AST â†’ write files â†’ update sandbox
Code â†’ Canvas: File watcher pada packages/workshop/sandbox/output/ â†’ parse AST â†’ update node properties/position (jika struktur berubah)
...
Sequence diagram untuk: "User minta bikin fitur baru" â†’ Survey â†’ Storm â†’ Review â†’ Canvas Update
...
Pilih battles: Workshop punya banyak fitur sexy (AI, canvas, sync). Prioritaskan yang membuat "ide â†’ sistem" menjadi nyata untuk user awam.
...
Klik node "Halaman Kasir" â†’ melihat preview yang bisa diklik
Chat: "Tambahkan tombol diskon 10%" â†’ melihat node baru muncul dan tersambung
Klik "Export" â†’ mendapat ZIP file yang bisa dibuka developer untuk di-deploy
